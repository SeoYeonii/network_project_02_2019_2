	...
	public void run() {
		...
		try {
			...
			while(true)
			{
				...
				if(text == null)
				{
					System.out.println(m_ID + "님이 <퇴장> 하셨습니다.");
					for(int i = 0; i < ChatServer.m_OutputList.size(); ++i)
					{
						ChatServer.m_OutputList.get(i).println(m_ID + "님이 <퇴장> 하셨습니다.");
				...
				{
					m_ID = split[1];
					System.out.println(m_ID + "님이 <입장> 하셨습니다.");
					for(int i = 0; i < ChatServer.m_OutputList.size(); ++i)
					{
						ChatServer.m_OutputList.get(i).println(m_ID + "님이 <입장> 하셨습니다.");
						ChatServer.m_OutputList.get(i).flush();
					}
					continue;
				}
